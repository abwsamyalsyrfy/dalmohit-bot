...
# تحميل متغيرات البيئة من ملف .env محلياً إن وجد (لا يتجاوز المتغيرات المضبوطة مسبقاً)
...
async def error_handler(update: object, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    """يلتقط أي خطأ غير متوقع ويسجله دون إيقاف البوت"""
    logger.error("خطأ غير متوقع أثناء معالجة التحديث:", exc_info=ctx.error)
    if update and isinstance(update, Update) and update.effective_message:
        try:
            await update.effective_message.reply_text(
                "⚠️ حدث خطأ غير متوقع. يرجى المحاولة مرة أخرى أو كتابة /start للعودة للقائمة الرئيسية."
            )
        except Exception:
            pass
...
def main():
    ...
    # ─── استمرارية بيانات المستخدمين (إشارات، مفضلة، تقدم القراءة) بين إعادات التشغيل ───
    persistence = PicklePersistence(filepath=os.path.join(DATA_DIR, "user_persistence.pkl"))
    ...
    # معالج الأخطاء العامة — يمنع توقف البوت عند أي خطأ غير متوقع
